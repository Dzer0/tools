...
# 以下表示替换各种关键字
sed -i 's%http://127.0.0.1:2379%'$1'%g' /etc/sysconfig/flanneld # 表示替换本机ip
# sed -i 's%ETCD_LIST%'$1'%g' /etc/sysconfig/flanneld # 表示替换本机ip
sed -i 's%/atomic.io/network%'$2'%g' /etc/sysconfig/flanneld  # 表示替换所有etcd集群列表
sed -i 's%#FLANNEL_OPTIONS=""%FLANNEL_OPTIONS="-iface=eno16777736"%g' /etc/sysconfig/flanneld  # 表示替换所有etcd集群列表
